         ...
         <el-table-column type="expand" width="180"> 
             <template slot-scope="scope">
                 ...
                 <el-row v-for="(item1,i) in scope.row.children" :key="i">
                     <el-col :span="4">
                         <!-- 传值 -->
                         <el-tag @close="deleRole(scope.row.id,item1.id)" class="roleslist" closable>{{item1.authName}}</el-tag>
                     ...
                     <el-col :span="20">
                         ...
                         <el-row v-for="(item2,i) in item1.children" :key="i">
                             <el-col :span="4">
                                 <el-tag @close="deleRole(scope.row.id,item2.id)" class="roleslist" type="success" closable>{{item2.authName}}</el-tag>
                                 <i class="el-icon-arrow-right"></i>
                             </el-col>
                             <!-- 展开三级权限 -->
                             <el-col :span="20">
                                 <el-tag @close="deleRole(scope.row.id,item3.id)" class="roleslist" type="warning" closable v-for="(item3,i) in item2.children" :key="i">{{item3.authName}}</el-tag>
                             </el-col>
                         </el-row>
                     </el-col>
                 </el-row>
